...
lambda0 = 1300e-9; f = c/lambda0;

n0 = 3.5; % Parte real
n1 = 2.069e-3; 
n2 = c*inv(4*pi*f)*g; % Parte imaginaria 
...
LAMBDA = lambda0/2/n0; % Formula de problemas

% Muestreo de la perturbacion

    % Periodo de muestreo - con 11 o 13 basta
    z_muestreo = LAMBDA/13;
    
    vector_z = linspace(0,Lper,Lper/z_muestreo);
    n_z = [n0-1j*0, n0 + n1.*sin(2*pi*inv(LAMBDA).*vector_z) + 1j*n2, n0-1j*0];
...
%% Caracterizacion macroscopica dispositivo:
    ...
    f_i = linspace(f-2e12, f+2e12, N_frec); % Vector fila
